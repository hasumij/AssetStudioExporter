...
{
    public class SpriteHelper(AssetsFileInstance inst, AssetsManager am)
    {
        public Image<Bgra32>? GetImage(Sprite m_Sprite)
        {
            if (m_Sprite.m_SpriteAtlas != null && m_Sprite.m_SpriteAtlas.TryFindInstance<SpriteAtlas>(out var m_SpriteAtlas, inst, am))
            {
                if (m_SpriteAtlas.m_RenderDataMap.TryGetValue(m_Sprite.m_RenderDataKey, out var spriteAtlasData) && spriteAtlasData.texture.TryFindInstance<Texture2D>(out var m_Texture2D, inst, am))
                {
            ...
            {
                if (m_Sprite.m_RD.texture.TryFindInstance<Texture2D>(out var m_Texture2D, inst, am))
                {
        ...
        private Image<Bgra32>? CutImage(Sprite m_Sprite, Texture2D m_Texture2D, Rectf textureRect, Vector2 textureRectOffset, float downscaleMultiplier, SpriteSettings settingsRaw)
        {
            var originalImage = m_Texture2D.GetImage(inst, false);
            ...
            using (originalImage)
            {
                ...
                if (settingsRaw.packingMode == SpritePackingMode.Tight)
                {
                    try
                    {
                        ...
                        var matrix = System.Numerics.Matrix3x2.CreateScale(m_Sprite.m_PixelsToUnits);
                        matrix *= System.Numerics.Matrix3x2.CreateTranslation(m_Sprite.m_Rect.width * m_Sprite.m_Pivot.X - textureRectOffset.X, m_Sprite.m_Rect.height * m_Sprite.m_Pivot.Y - textureRectOffset.Y);
